...
def allcounts(conc, x0, xctrl, GRmax, x50, Hill):
    ...
    deadcells = add_count_noise(deadcount(conc, dead_max, x50+.1, Hill, dead_min))/2
    viablecells = totalcells - deadcells
    corpses = add_count_noise(np.maximum(10, x0-totalcells)+10)+deadcells

    
    return pd.DataFrame([[conc, totalcells, deadcells+corpses, viablecells, corpses]],
                        columns=['concentration', 'cell_count__total','cell_count__dead','cell_count','cell_count__corpse']) 
       # return pd.DataFrame(dict=({conc, totalcells, deadcells+corpses, viablecells, corpses],
        #                columns=['concentration', 'cell_count__total','cell_count__dead','cell_count','cell_count__corpse']) 
...
def Columbus_fixed(df_trt, df_gr, df_sens):
    ...
    df_res.rename(index=str, columns={'cell_count__total': 'Hoechst_pos_LDR_neg',
                                      'cell_count__dead': 'Hoechst_LDR_pos',
                                      'cell_count__corpse': 'LDR_pos_Hoechst_neg'},
                  inplace=True)
    return df_res
